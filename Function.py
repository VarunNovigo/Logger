          ...
          if isinstance(value, str):
              ...
              if match :
                ...
                if datee > date and bool1:
                  ans.append(f"{temp_val}, {key} : {value[:10]} {value[11:19]}")
...
class query:
    ...
    def main_query(self, dayss):
      ...
      for i, data in enumerate(top10_latencies, 1):
        ...
        ans_list.append(f"Username: {data['username']}, Query: {self.get_Query(data['path'])}, Latency: {data['latency']}")
      return ans_list
      ...
      for key, val in data.items():
          if cnt < 10:
              ...
              if date_time_obj.date() > date and date_time_obj.date() not in seen_dates:
                  ...
                  ans.append(f"Timestamp : {date_time_obj}, CPU Usage : {val[11:]}")
                  cnt = cnt + 1
      ...
      for key, val in data.items():
        if cnt < 10:
          ...
          if date_time_obj.date() > date and str(date_time_obj.strftime("%Y-%m-%d")+val) not in seen_dates:
            # print(date_time_obj, ' - ', val, "\n")
            ans.append(f"Timestamp : {date_time_obj}, Memory Usage : {val[9:]} \n")
            cnt = cnt + 1
...
class UserAction:
    ...
    def main_user_action(self, user, dayss, action):
      ...
      if matching_results:
          matching_results.pop(0)
          for i, data in enumerate(matching_results, 1):
            ans.append(f"Username: {data['userName']}, Timestamp: {data['timestamp']}, Action: {data['actionName']}, Message: {data['message']}")
...
# Top actions
    ...
    def most_user_actions(self, data):
      ...
      for val in sorted_dict:
          ...
          str = f"Action : {val}, Frequency : {sorted_dict[val]}"
          ans.append(str)
          cnt += 1
      return ans
            ...
            if flag:
              for item in lines:
                ...
                if match:
                  ...
                  if not ans.get(date_time_obj) and date_time_obj.date() > date:
                    ans[date_time_obj] =  "started"
                    anss.append(f"Timestamp : {date_time_obj}, Status : started")
                    break
            else:
              for item in lines:
              ...
              for item in lines:
                ...
                if match:
                  ...
                  if not ans.get(date_time_obj) and date_time_obj.date() > date:
                    ...
                    anss.append(f"Timestamp : {date_time_obj}, Status : stopped")
                    break
          except Exception as e:
              print(f"Error reading file {file_path}: {e}")
    return anss
